...
function MyPlaylistTracks(props) {
  ...
  useEffect(() => {
    ...
    if (!props.myPlaylistTracksCount)
      sethasMore(false)
    else if (props.tracks.length + tracks.length >= props.myPlaylistTracksCount) {
      sethasMore(false)
    } else {
      sethasMore(true)
    }
  ...
  return (
    <div className={search.tracksWrapper}>
      <div className={search.tracksHeading}>
        <h2>{props.tracksMeta ? "Tracks" : "Playlist Tracks"} {props.tracks.length > 0 && <span className={search.tracksCount}>({props.tracksMeta ? props.tracksMeta.total_track_count : props.myPlaylistTracksCount})</span>}</h2>
      ...
      {isLoading ? 
        ...
        <div className="trackRowWrapper">
          ...
          >
              ...
              <div className="trackRow" key={index}>
                ...
                {props.showDeleteButton && <OverlayTrigger overlay={<Tooltip>Remove From Playlist</Tooltip>}>
                  ...
                  <OverlayTrigger overlay={<Tooltip>Similar Search</Tooltip>}>
                    <a onClick={() => props.handleSimilarSearch(track.mediable.title, track.mediable.aims_id)}>
                  ...
                  <OverlayTrigger overlay={<Tooltip>Download Track</Tooltip>}>
                    <a onClick={() => {props.showDownloadModal(index, track.mediable_type);}}>
                  ...
                  {props.type !== "myplaylist" && <OverlayTrigger overlay={<Tooltip>Add to Playlist</Tooltip>}>
                    <a onClick={() => props.showTrackAddToPlaylistModal(index, track.mediable_type)}>
            ...
            )) : tracks.length == 0 && <div className="textCenter">No tracks found</div>}
          </InfiniteScroll>
        </div>
      }
